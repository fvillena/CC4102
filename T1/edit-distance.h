...
unsigned short compute_southeast_edge(char *X, char *Y, unsigned short i, unsigned short j)
{
    char x = X[i];
    char y = Y[j];
    // printf("%c%c ", x, y);
...
unsigned short dynamic_programming_algorithm(char *X, char *Y)
{
    ...
    for (unsigned short i = 1; i < n; i++)
    {
        for (unsigned short j = 1; j < n; j++)
        {
            ...
            unsigned short southeast_edge = compute_southeast_edge(X, Y, i - 1, j - 1);
            // printf("%d%d ", up, left);
...
unsigned short compute_cell(unsigned short northeast, unsigned short west, unsigned short north, unsigned short southeast_edge)
{
...
unsigned short in_cache_algorithm(char *X, char *Y)
{
    ...
    for (unsigned short k = 1; k < n; k++)
    {
        for (unsigned short i = k; i < n; i++)
        {
            ...
            unsigned short southeast_edge = compute_southeast_edge(X, Y, k-1, i-1);
            unsigned short cell = compute_cell(northeast, west, north, southeast_edge);
        ...
        for (unsigned short j = k + 1; j < n; j++)
        {
            ...
            unsigned short southeast_edge = compute_southeast_edge(X, Y, j-1, k-1);
            unsigned short cell = compute_cell(northeast, west, north, southeast_edge);
            // printf("[%d, %d] = %d\n", j, k, cell);
            last_north = cell;
